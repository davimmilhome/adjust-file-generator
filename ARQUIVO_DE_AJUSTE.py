...
import sys, datetime as dt, os, shutil as sh
...
data_atual = dt.date.today().strftime('%d%m%Y')    #INFORMAÇÃO DA DATA ATUAL PARA NOMEAR O ARQUIVO
...
def f_ajuste_cnpj():
    ...
    #Esse bloco confere 14 dígitos ao CNPJ (adição do zero inicial em alguns casos)
...
def f_ajuste_valor():
    ...
    #Bloco de encerramento
    print(f'O valor do ajuste é R$ {nu_valor[:len(nu_valor)-2]+ "," + nu_valor[len(nu_valor)-2:]}')

def f_data_mov():

    global dt_data_mov

    dt_data_mov = input('A data movimento (data ajuste) corresponde ao dia de hoje?\n'
...
def f_new_ajuste():
    ...
    new_ajuste = input('Gostaria de adicionar um novo ajuste?\n1- Sim\n2- Não\n')
